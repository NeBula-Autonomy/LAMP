...
TEST_F(TestPoseGraphClass, TestInitialize) {
  ...
  EXPECT_EQ(pose_graph_.GetNodes().size(), 1);
  EXPECT_EQ(pose_graph_.GetNfg().size(), 1);
  EXPECT_EQ(pose_graph_.GetEdges().size(), 0);
  EXPECT_EQ(pose_graph_.GetPriors().size(), 1);
...
TEST_F(TestPoseGraphClass, TestTrackNode) {
  ...
  EXPECT_EQ(pose_graph_.GetNodes().size(), 3);
  EXPECT_EQ(pose_graph_.GetNfg().size(), 1);
  EXPECT_EQ(pose_graph_.GetEdges().size(), 0);
  EXPECT_EQ(pose_graph_.GetPriors().size(), 1);
...
TEST_F(TestPoseGraphClass, TestTrackEdges) {
  ...
  EXPECT_EQ(pose_graph_.GetEdges().size(), 2);
  EXPECT_EQ(pose_graph_.GetPriors().size(), 1);
}
...
TEST_F(TestPoseGraphClass, Multi_robot_add){
  ...
  EXPECT_EQ(pose_graph_.GetNodes().size(), 5);
  EXPECT_EQ(pose_graph_.GetNfg().size(), 5);
  EXPECT_EQ(pose_graph_.GetEdges().size(), 3);
  EXPECT_EQ(pose_graph_.GetPriors().size(), 2);
}
...
TEST_F(TestPoseGraphClass, RemoveRobotFromGraph){
  ...
  // Make a copy 
  PoseGraph pose_graph_back = pose_graph_;
  ...
  EXPECT_EQ(pose_graph_.GetNodes().size(), 2);
  EXPECT_EQ(pose_graph_.GetNfg().size(), 2);
  EXPECT_EQ(pose_graph_.GetEdges().size(), 1);
  EXPECT_EQ(pose_graph_.GetPriors().size(), 1);
  ...
  EXPECT_EQ(pose_graph_back.GetNodes().size(), 3);
  EXPECT_EQ(pose_graph_back.GetNfg().size(), 3);
  EXPECT_EQ(pose_graph_back.GetEdges().size(), 2);
  EXPECT_EQ(pose_graph_back.GetPriors().size(), 1);
}
