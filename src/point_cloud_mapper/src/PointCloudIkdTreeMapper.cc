...
// bool PointCloudIkdTreeMapper::InsertPoints(const PointCloud::ConstPtr&
// points,
//                                           PointCloud* incremental_points) {
//  if (!initialized_) {
//    ROS_ERROR("%s: Not initialized.", name_.c_str());
...
//  no_points.data = points->size();
//  insert_points_init_pub_.publish(no_points);
//  // Try to get the map mutex from the publisher. If the publisher is using
//  it,
...
//      PointToAdd.reserve(points->points.size());
//      PointNoNeedDownsample.reserve(points->points.size());
...
//        mid_point.x = floor(points->points[i].x / filter_size_map_min) *
//                filter_size_map_min +
//            0.5 * filter_size_map_min;
//        mid_point.y = floor(points->points[i].y / filter_size_map_min) *
//                filter_size_map_min +
//            0.5 * filter_size_map_min;
//        mid_point.z = floor(points->points[i].z / filter_size_map_min) *
//                filter_size_map_min +
//            0.5 * filter_size_map_min;
//        float dist = calc_dist(points->points[i], mid_point);
//        if (fabs(points->points[i].x - mid_point.x) >
//                0.5 * filter_size_map_min &&
//            fabs(points->points[i].y - mid_point.y) >
//                0.5 * filter_size_map_min &&
//            fabs(points->points[i].z - mid_point.z) >
//                0.5 * filter_size_map_min) {
//#pragma omp critical
//          { PointNoNeedDownsample.push_back(points->points[i]); }
//          continue;
//        }

//        if (calc_dist(points->points[i], mid_point) < dist) {
//          need_add = false;
//          continue;
//        }
...
//        if (need_add) {
//#pragma omp critical
//          { PointToAdd.push_back(points->points[i]); }
//        }
//      }
//      int add_point_size = ikdtree.Add_Points(PointToAdd, true);
//      ikdtree.Add_Points(PointNoNeedDownsample, false);
//      add_point_size = PointToAdd.size() + PointNoNeedDownsample.size();
//      ROS_INFO_STREAM("ADDING POINTS: " << add_point_size << " "
//                                        << PointToAdd.size() << " "
//                                        << PointNoNeedDownsample.size());
//      // double st_time = omp_get_wtime();

//      //      int add_point_size = ikdtree.Add_Points(PointToAdd, true);
//      //      ikdtree.Add_Points(PointNoNeedDownsample, false);
//      //      add_point_size = PointToAdd.size() +
//      //      PointNoNeedDownsample.size();
...
#pragma omp parallel for schedule(dynamic, 1)
  for (size_t ii = 0; ii < points.points.size(); ++ii) {
    ...
    //#pragma omp critical
    //    std::cout << "num: " << omp_get_thread_num()
    //              << " => tid: " << syscall(__NR_gettid) << std::endl;
    PointVector search_result;
    vector<float> PointDist;
    ikdtree.Nearest_Search(points[ii], 1, search_result, PointDist);
    // neighbors->push_back(search_result[0]);
