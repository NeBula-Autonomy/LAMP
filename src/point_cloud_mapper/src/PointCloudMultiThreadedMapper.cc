...
bool PointCloudMultiThreadedMapper::InsertPoints(const PointCloud::ConstPtr& points,
  ...
  if (refresh_id_ == "a") {
    ...
    if (map_mutex_.try_lock()) {
      ...
      if (b_add_history_to_a_) {
        ...
        std::chrono::duration<double> add_history_duration = add_history_end_time - add_history_start_time;
        ROS_INFO_STREAM("Added history in " << add_history_duration.count() << " s");
        history_->clear();
      ...
      for (size_t ii = 0; ii < points->points.size(); ++ii) {
        ...
        if (!isInBox || !map_octree_->isVoxelOccupiedAtPoint(p)) {
          ...
          if (b_keep_history_) history_->push_back(p);  // This history is produced to be then added to A 
        }
      }
    ...
    if (map_mutex_b_.try_lock()) {
      ...
      for (size_t ii = 0; ii < points->points.size(); ++ii) {
        ...
        if (!isInBox || !map_octree_b_->isVoxelOccupiedAtPoint(p)) {
          map_octree_b_->addPointToCloud(p, map_data_b_);
          incremental_points->push_back(p);  
          if (b_keep_history_) history_->push_back(p);  // This history is produced to be then added to B      
        }
      }
...
bool PointCloudMultiThreadedMapper::ApproxNearestNeighbors(const PointCloud& points,
  ...
  for (size_t ii = 0; ii < points.points.size(); ++ii) {
    ...
    if (refresh_id_ == "a") {
      ...
      map_octree_b_->approxNearestSearch(points.points[ii], result_index, unused);
      if (result_index >= 0)
        neighbors->push_back(map_data_b_->points[result_index]);
    }
...
void PointCloudMultiThreadedMapper::PublishMapFrozen() {
  if (initialized_ && map_frozen_pub_.getNumSubscribers() > 0) {
    ...
    publish_frozen_thread_ = std::thread(&PointCloudMultiThreadedMapper::PublishMapFrozenThread, this);
  }
}
...
void PointCloudMultiThreadedMapper::PublishMapUpdate(const PointCloud& incremental_points) {
...
void PointCloudMultiThreadedMapper::Refresh(const geometry_utils::Transform3& current_pose) {
  ...
  refresh_thread_ = std::thread(&PointCloudMultiThreadedMapper::RefreshThread, this);
