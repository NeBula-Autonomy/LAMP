...
void KD_TREE::InitializeKDTree(float delete_param,
                               float balance_param,
                               float box_length) {
  ...
  while (!terminated) {
    ...
    if (Rebuild_Ptr != nullptr) {
      ...
      if (Rebuild_Logger.size() > 0) {
        printf("\n\n\n\n\n\n\n\n\n\n\n ERROR!!! \n\n\n\n\n\n\n\n\n");
        std::cout << "In the meantime there were incremental operations!"
                  << std::endl;
      }
      rebuild_flag = true;
      ...
      if (int(Rebuild_PCL_Storage.size()) > 0) {
        BuildTree(&new_root_node,
                  0,
                  Rebuild_PCL_Storage.size() - 1,
                  Rebuild_PCL_Storage);
...
void KD_TREE::run_operation(KD_TREE_NODE** root,
                            Operation_Logger_Type operation) {
  switch (operation.op) {
  case ADD_POINT:
...
void KD_TREE::Build(PointVector point_cloud) {
  ...
  BuildTree(
      &STATIC_ROOT_NODE->left_son_ptr, 0, point_cloud.size() - 1, point_cloud);
  Update(STATIC_ROOT_NODE);
  STATIC_ROOT_NODE->TreeSize = 0;
  Root_Node = STATIC_ROOT_NODE->left_son_ptr;
}

void KD_TREE::Nearest_Search(PointType point,
                             int k_nearest,
...
void KD_TREE::Add_Points(PointVector& PointToAdd, bool downsample_on) {
  ...
  int tree_size = size();
  if (tree_size > 0 && NewPointSize > Multi_Thread_Rebuild_Point_Num &&
      float(NewPointSize) / float(tree_size) > ForceRebuildPercentage) {
    pthread_mutex_lock(&working_flag_mutex);
...
void KD_TREE::BuildTree(KD_TREE_NODE** root,
                        int l,
                        int r,
                        PointVector& Storage) {
  if (l > r)
  ...
  *root = new KD_TREE_NODE;
  ...
  for (i = 0; i < 3; i++)
    covariance[i] = covariance[i] / (r - l + 1);
  int div_axis = 0;
  for (i = 1; i < 3; i++) {
    if (covariance[i] > covariance[div_axis])
      div_axis = i;
  }
  (*root)->division_axis = div_axis;
  switch (div_axis) {
  case 0:
    nth_element(begin(Storage) + l,
                begin(Storage) + mid,
                begin(Storage) + r + 1,
                point_cmp_x);
    break;
...
void KD_TREE::Delete_by_range(KD_TREE_NODE** root,
  ...
  if ((*root) == nullptr || (*root)->tree_deleted)
    return;
  Push_Down(*root);
  if (boxpoint.vertex_max[0] + EPSS < (*root)->node_range_x[0] ||
      boxpoint.vertex_min[0] - EPSS > (*root)->node_range_x[1])
    return;
  if (boxpoint.vertex_max[1] + EPSS < (*root)->node_range_y[0] ||
      boxpoint.vertex_min[1] - EPSS > (*root)->node_range_y[1])
    return;
  if (boxpoint.vertex_max[2] + EPSS < (*root)->node_range_z[0] ||
      boxpoint.vertex_min[2] - EPSS > (*root)->node_range_z[1])
    return;
  if (boxpoint.vertex_min[0] - EPSS < (*root)->node_range_x[0] &&
      boxpoint.vertex_max[0] + EPSS > (*root)->node_range_x[1] &&
      boxpoint.vertex_min[1] - EPSS < (*root)->node_range_y[0] &&
      boxpoint.vertex_max[1] + EPSS > (*root)->node_range_y[1] &&
      boxpoint.vertex_min[2] - EPSS < (*root)->node_range_z[0] &&
      boxpoint.vertex_max[2] + EPSS > (*root)->node_range_z[1]) {
  ...
  if (boxpoint.vertex_min[0] - EPSS < (*root)->point.x &&
      boxpoint.vertex_max[0] + EPSS > (*root)->point.x &&
      boxpoint.vertex_min[1] - EPSS < (*root)->point.y &&
      boxpoint.vertex_max[1] + EPSS > (*root)->point.y &&
      boxpoint.vertex_min[2] - EPSS < (*root)->point.z &&
      boxpoint.vertex_max[2] + EPSS > (*root)->point.z) {
    (*root)->point_deleted = true;
    if (is_downsample)
      (*root)->point_downsample_deleted = true;
  }
  ...
  if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr) {
    Delete_by_range(
        &((*root)->left_son_ptr), boxpoint, allow_rebuild, is_downsample);
  } else {
    pthread_mutex_lock(&working_flag_mutex);
  ...
  if ((Rebuild_Ptr == nullptr) || (*root)->right_son_ptr != *Rebuild_Ptr) {
    Delete_by_range(
        &((*root)->right_son_ptr), boxpoint, allow_rebuild, is_downsample);
  } else {
    pthread_mutex_lock(&working_flag_mutex);
...
void KD_TREE::Delete_by_point(KD_TREE_NODE** root,
                              PointType point,
                              bool allow_rebuild) {
  if ((*root) == nullptr || (*root)->tree_deleted)
    return;
...
void KD_TREE::Add_by_range(KD_TREE_NODE** root,
                           BoxPointType boxpoint,
                           bool allow_rebuild) {
  if ((*root) == nullptr)
    return;
...
void KD_TREE::Add_by_point(KD_TREE_NODE** root,
                           PointType point,
                           bool allow_rebuild) {
  if (*root == nullptr) {
    *root = new KD_TREE_NODE;
...
void KD_TREE::Search(KD_TREE_NODE* root,
                     int k_nearest,
                     PointType point,
...
void KD_TREE::Search_by_range(KD_TREE_NODE* root,
                              BoxPointType boxpoint,
                              PointVector& Storage) {
  if (root == nullptr)
    return;
...
void KD_TREE::Push_Down(KD_TREE_NODE* root) {
  ...
  if (root->need_push_down_to_left && root->left_son_ptr != nullptr) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != root->left_son_ptr) {
      ...
      root->left_son_ptr->point_deleted = root->left_son_ptr->tree_deleted ||
          root->left_son_ptr->point_downsample_deleted;
  ...
  if (root->need_push_down_to_right && root->right_son_ptr != nullptr) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != root->right_son_ptr) {
      ...
      root->right_son_ptr->point_deleted = root->right_son_ptr->tree_deleted ||
          root->right_son_ptr->point_downsample_deleted;
...
bool KD_TREE::point_cmp_x(PointType a, PointType b) {
  return a.x < b.x;
}
bool KD_TREE::point_cmp_y(PointType a, PointType b) {
  return a.y < b.y;
}
bool KD_TREE::point_cmp_z(PointType a, PointType b) {
  return a.z < b.z;
}

void KD_TREE::print_tree(int index,
                         FILE* fp,
                         float x_min,
                         float x_max,
                         float y_min,
  ...
  pthread_mutex_lock(&working_flag_mutex);
  print_treenode(
      Root_Node, index, fp, x_min, x_max, y_min, y_max, z_min, z_max);
  pthread_mutex_unlock(&working_flag_mutex);
}

void KD_TREE::print_treenode(KD_TREE_NODE* root,
                             int index,
                             FILE* fp,
                             float x_min,
                             float x_max,
  ...
  case 0:
    print_treenode(root->left_son_ptr,
                   index,
                   fp,
                   x_min,
                   root->point.x,
                   ...
                   z_min,
                   z_max);
    print_treenode(root->right_son_ptr,
                   index,
                   fp,
                   root->point.x,
                   x_max,
...
void KD_TREE::Delete_Point_Boxes_NEGATIVE(vector<BoxPointType>& BoxPoints) {
  for (int i = 0; i < BoxPoints.size(); i++) {
    if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node) {
      Delete_by_range_NEGATIVE(&Root_Node, BoxPoints[i], true, false);
    } else {
      Operation_Logger_Type operation;
      operation.boxpoint = BoxPoints[i];
      operation.op = DELETE_BOX;
      pthread_mutex_lock(&working_flag_mutex);
      Delete_by_range_NEGATIVE(&Root_Node, BoxPoints[i], false, false);
      if (rebuild_flag) {
        pthread_mutex_lock(&rebuild_logger_mutex_lock);
        Rebuild_Logger.push(operation);
        pthread_mutex_unlock(&rebuild_logger_mutex_lock);
...
void KD_TREE::Delete_by_range_NEGATIVE(KD_TREE_NODE** root,
                                       BoxPointType boxpoint,
                                       bool allow_rebuild,
                                       bool is_downsample) {
  if ((*root) == nullptr || (*root)->tree_deleted)
  ...
  delete_box_log.boxpoint = boxpoint;
  if ((Rebuild_Ptr == nullptr) || (*root)->left_son_ptr != *Rebuild_Ptr) {
    Delete_by_range_NEGATIVE(
        &((*root)->left_son_ptr), boxpoint, allow_rebuild, is_downsample);
  } else {
    pthread_mutex_lock(&working_flag_mutex);
    Delete_by_range_NEGATIVE(
        &((*root)->left_son_ptr), boxpoint, false, is_downsample);
    if (rebuild_flag) {
      pthread_mutex_lock(&rebuild_logger_mutex_lock);
      Rebuild_Logger.push(delete_box_log);
      pthread_mutex_unlock(&rebuild_logger_mutex_lock);
  ...
  if ((Rebuild_Ptr == nullptr) || (*root)->right_son_ptr != *Rebuild_Ptr) {
    ...
    pthread_mutex_lock(&working_flag_mutex);
    Delete_by_range_NEGATIVE(
        &((*root)->right_son_ptr), boxpoint, false, is_downsample);
    if (rebuild_flag) {
      pthread_mutex_lock(&rebuild_logger_mutex_lock);
      Rebuild_Logger.push(delete_box_log);
      pthread_mutex_unlock(&rebuild_logger_mutex_lock);
