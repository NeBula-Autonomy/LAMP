...
class PoseGraphModifier(object):
    ...
    def modify_pg(self, pg):
        ...
        # # Hack: remove false loop closure edges connected to artifact
        # artifact_key = []
        # for edge in pg.edges:
        #     if edge.type == PoseGraphEdge.ARTIFACT:
        #         artifact_key.append(edge.key_to)
        #
        # modified_pg = PoseGraph()
        # modified_pg.header = copy.deepcopy(pg.header)
        # modified_pg.nodes = copy.deepcopy(pg.nodes)
        # for edge in pg.edges:
        #     if edge.type == PoseGraphEdge.LOOPCLOSE and edge.key_to in artifact_key:
        #             continue
        #     modified_pg.edges.append(edge)
        # return copy.deepcopy(modified_pg)
...
class PoseGraphHandler(object):
    ...
    def optimize(self, pg):
        ...
        # for itr in range(len(pub_pg.edges)):
        #     if pub_pg.edges[itr].type == PoseGraphEdge.ARTIFACT:
        #         pub_pg.edges[itr].type == PoseGraphEdge.LOOPCLOSE
