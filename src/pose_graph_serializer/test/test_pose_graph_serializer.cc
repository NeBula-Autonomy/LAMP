...
TEST_F(TestSerializer, BasicMerge) {
  ...
  // Give graphs to Serializer
  Serializer.OnSlowGraphMsg(slow_graph);
  Serializer.OnFastGraphMsg(fast_graph);

  // Get the result
  pose_graph_msgs::PoseGraph current_graph = Serializer.GetCurrentGraph();
...
TEST_F(TestSerializer, FastGraphOnly) {
  ros::NodeHandle nh, pnh("~");
...
TEST_F(TestSerializer, MergeWithValuesOnlyInSlowGraph) {
  ros::NodeHandle nh, pnh("~");
...
TEST_F(TestSerializer, MergeWithEmptySlowGraph) {
  ros::NodeHandle nh, pnh("~");
  ...
  // Check that final pose is correct
  EXPECT_NEAR(8.0, x, tolerance_);
  EXPECT_NEAR(0.0, y, tolerance_);
  EXPECT_NEAR(0.0, z, tolerance_);
}


TEST_F(TestSerializer, DoubleMerge) {
  ros::NodeHandle nh, pnh("~");
  ...
  Serializer.OnFastGraphMsg(fast_graph_1);
  Serializer.OnSlowGraphMsg(slow_graph_1);
  Serializer.OnFastGraphMsg(fast_graph_2);
  pose_graph_msgs::PoseGraph current_graph = Serializer.GetCurrentGraph();
  ...
  Serializer.OnSlowGraphMsg(slow_graph_2);
  Serializer.OnFastGraphMsg(fast_graph_3);
  current_graph = Serializer.GetCurrentGraph();
...
TEST_F(TestSerializer, MergeWithArtifactInFastGraph) {
  ros::NodeHandle nh, pnh("~");
...
int main(int argc, char** argv) {
  ...
  ros::init(argc, argv, "test_pose_graph_Serializer");
