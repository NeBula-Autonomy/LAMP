...
public:
  ...
  ~TestArtifactHandler() {}
  bool LoadParameters() {
    return art_handle.LoadParameters(nh);
  }
  bool RegisterCallbacks() {
    return art_handle.RegisterCallbacks(nh, false);
  };
  bool UpdateGlobalPose(gtsam::Symbol artifact_key, gtsam::Pose3 global_pose) {
    art_handle.UpdateGlobalPose(artifact_key, global_pose);
  };
  FactorData& GetArtifactData() {
    return art_handle.artifact_data_;
  };
  FactorData GetData() {
    return art_handle.GetData();
  };
  bool ArtifactCallback(core_msgs::Artifact msg) {
    art_handle.ArtifactCallback(msg);
  };
  std::unordered_map<long unsigned int, ArtifactInfo> GetKeyInfoMap() {
    return art_handle.artifact_key2info_hash_;
  };
  std::unordered_map<std::string, gtsam::Symbol> GetStringKeyMap() {
    return art_handle.artifact_id2key_hash;
  };
};
