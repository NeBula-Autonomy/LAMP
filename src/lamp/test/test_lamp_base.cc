...
TEST_F(TestLampBase, AddScansBeforePoseNodes){
  ...
  pose_graph_msgs::KeyedScan::ConstPtr msg_ptr(new pose_graph_msgs::KeyedScan(scan_msg_));
  data_.scans.push_back(msg_ptr);

  // Process data
  std::shared_ptr<PoseGraphData> data_shared = std::make_shared<PoseGraphData>(data_);
  ...
  pose_graph_msgs::PoseGraph::ConstPtr msg_ptr2(new pose_graph_msgs::PoseGraph(graph_));
